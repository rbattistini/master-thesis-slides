...
- augments the existing BDI agents rather than replacing them;
  - unlike LLM agents, which are fully generative and language-driven.
...
#grid(
  ...
  gutter: 1em,
  [
    The BDI developer configures the generation process and describes the plans, rather than encode them, by:
...
#raw("!reach(home): there_is(home, here).
...
#columns(1)[
  ...
  #set align(left)
  LLMs are promising for plan generation, provided that appropriate parameter configurations are given and the the BDI programmer documents the domain with hints. 
